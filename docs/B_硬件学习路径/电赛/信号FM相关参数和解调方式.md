...
![](/image_photo/AMFMDSB/lc鉴频.jpg)

![](/image_photo/AMFMDSB/fmlcam.jpg)
...
#### 核心设计要求与物理机制
设计目标：使相位差 $\Delta \phi$ 与角频率偏移 $\Delta \omega$（$\Delta \omega = 2\pi\Delta f$）满足线性关系 $\Delta \phi = k \cdot \Delta \omega$（$k$ 为比例常数），通过以下两步实现：
...
- 当 $\omega = \omega_c$ 时：次级电压与初级电压呈预设的 90° 相移（正交）；
...
#####  信号叠加原理
设两路信号幅度相等（工程中通过匹配确保 $A = B$）：
- 参考信号：$V_1(t) = A \cdot \cos[\omega t + \phi(t)]$（$\phi(t)$ 为FM信号固有相位）；
